...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Your learning command center." },
    ],
  }),
  component: Dashboard,
});
          ...
          <div className="relative flex flex-wrap items-end justify-between gap-4">
            <div>
              <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-primary">
                ...
                {CURRENT_USER.subscription_status === "free" ? "Free Plan" : CURRENT_USER.subscription_status.toUpperCase() + " Plan"}
              </div>
              <h1 className="font-display text-3xl font-bold tracking-tight md:text-4xl">
                Welcome back, <span className="text-primary">{CURRENT_USER.name.split(" ")[0]}</span>.
              </h1>
              <p className="mt-2 text-sm text-muted-foreground">
                You've completed {completed.length} lessons. Keep the streak alive.
              </p>
            </div>
            <div className="flex gap-8">
              <Stat label="Lessons done" value={completed.length} />
              <Stat label="Courses active" value={enriched.filter(e => e.status !== "not-started").length} />
            </div>
          </div>
        </div>
        ...
        {resume && (
          <section className="mt-10">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-display text-xl font-semibold tracking-tight">Continue learning</h2>
              <span className="text-xs uppercase tracking-wider text-muted-foreground">Pick up where you left off</span>
            </div>
            <Link
              to="/learn/$courseId"
            ...
            >
              ...
              <div className="p-6 md:p-8">
                ...
                <div className="mt-5 flex items-center gap-4 text-xs text-muted-foreground">
                  <span className="inline-flex items-center gap-1.5"><Clock className="h-3.5 w-3.5" /> {formatDuration(resume.lesson.duration)} mins</span>
                  <span>·</span>
                  <span>{resume.course.title}</span>
                </div>
                <div className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-primary">
                  Resume lesson <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
                </div>
              </div>
            </Link>
          </section>
